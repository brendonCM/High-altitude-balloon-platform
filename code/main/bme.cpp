...
void BME::get_BME_Sensor_Values(float *extTemp, float *Ambientpressure, float *Relativehumidity)
{
	calcBME_Sensor_Values(fileNo);
	*extTemp = ET;
	*Ambientpressure = AP;
	*Relativehumidity = RH;
}
...
int BME::setupI2C()
{
	...
	// put the path to the I2C connection here
	char *bus = "/dev/i2c-1";
	if ((file_no = open(bus, O_RDWR)) < 0)
	{
	...
	ioctl(file_no, I2C_SLAVE, 0x77);
	return file_no;
}

void BME::calcBME_Sensor_Values(int file)
{
	...
	// T1 is an unsigned short and T2 to T3 are signed short parameters temp coefficents
