...
PWM::PWM(string pinName,int number, string chip) {
	this->name = pinName;
	this->number = number;
	this->chip = chip;
	this->path1 = PWM_PATH + this->chip + '/';
	this->exportPWM();
	this->path = PWM_PATH + this->chip + '/' +  this->name + '/';  
...
int PWM::exportPWM() {
	return write(this->path1, "export", this->number);
}
